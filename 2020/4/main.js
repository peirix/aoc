...
const required = [
	{ name: 'byr', type: 'num', min: 1920, max: 2002 },
	{ name: 'iyr', type: 'num', min: 2010, max: 2020 },
	{ name: 'eyr', type: 'num', min: 2020, max: 2030 },
	{
		name: 'hgt',
		type: 'height',
		in: { min: 59, max: 76 },
	...
	{ name: 'hcl', type: 'regex', regex: /^#[0-9a-f]{6}$/ },
	{
		name: 'ecl',
		type: 'color',
		valid: ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
	},
	{ name: 'pid', type: 'regex', regex: /^[0-9]{9}$/ },
	{ name: 'cid', type: 'skip' }
];
...
lines.forEach((line) => {
	...
	if (parts.length < required.length - 1) {
		// document.write(' missing 2 or more<br>');
		return;
	}
	...
	parts.forEach((part) => {
		reqParts++;
		const name = part.substr(0, 3);
		const rule = required.find((r) => r.name === name);
		let val = part.substr(4);
		if (rule.type === 'num') {
			val = parseInt(val);
			if (val < rule.min || val > rule.max) {
				isValid = false;
			} else {
				document.write('num: ' + val);
				document.write(
					' min: ' + rule.min + ' max: ' + rule.max + '<br>'
				);
			}
		} else if (rule.type === 'height') {
			const type = val.indexOf('cm') > -1 ? 'cm' : 'in';
			val = parseInt(val);
			if (val < rule[type].min || val > rule[type].max) {
				isValid = false;
			} else {
				document.write('height: ' + val + type);
				document.write(
					' min: ' +
						rule[type].min +
						' max: ' +
			...
			if (!val.match(rule.regex)) {
				isValid = false;
			} else {
				document.write('regex: ' + val);
				document.write(
					' rule ' +
						rule.regex +
						' match: ' +
						val.match(rule.regex)[0] +
						'<br>'
				);
			}
		} else if (rule.type === 'color') {
			if (!rule.valid.includes(val)) {
				isValid = false;
			} else {
				document.write('eye: ' + val);
				document.write(' ' + rule.valid.join('|'));
			}
		} else if (rule.type === 'skip') {
			reqParts--;
		}
	});
	if (isValid && reqParts === 7) {
		valid++;

		document.write(
			' <span style="color:green;font-weight:bold">valid</span><br><br>'
		);
	}
});
