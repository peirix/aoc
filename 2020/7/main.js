const input = `bright olive bags contain 5 dotted white bags, 2 wavy lavender bags.
...
lines.forEach((line) => {
	const color = line.split(' bags ')[0];
	const newBags = line.split(' contain ')[1];
	const tmp = [];
	newBags.split(',').forEach((b) => {
...
function getBagsIn(name) {
	const bag = bags.find((b) => b.name === name);
	console.log(bag, name);
	let total = 0;
	bag.bags.forEach((nested) => {
		total += nested.amt + getBagsIn(nested.name) * nested.amt;
	});
	return total;
}

console.log(getBagsIn('shiny gold', 1));
