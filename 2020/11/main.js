const input = `LLL.LLL.LLLLL.LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLL.LLLLLLLLL.LLLLLLLLLLLLLLLL.LLLLLL.LLLLLLLL
...
function getOccClose(y, x) {
	...
	for (let i = 1; i < 100; i++) {
		if (!found.u && y - i >= 0) {
			found.u = seats[y - i][x] !== '.' ? seats[y - i][x] : false;
			occ += seats[y - i][x] === '#' ? 1 : 0;
		}
		if (!found.d && y + i < rows) {
			found.d = seats[y + i][x] !== '.' ? seats[y + i][x] : false;
			occ += seats[y + i][x] === '#' ? 1 : 0;
		}
		if (!found.l && x - i >= 0) {
			found.l = seats[y][x - i] !== '.' ? seats[y][x - i] : false;
			occ += seats[y][x - i] === '#' ? 1 : 0;
		}
		if (!found.r && x + i < cols) {
			found.r = seats[y][x + i] !== '.' ? seats[y][x + i] : false;
			occ += seats[y][x + i] === '#' ? 1 : 0;
		}
		if (!found.ul && y - i >= 0 && x - i >= 0) {
			found.ul =
				seats[y - i][x - i] !== '.' ? seats[y - i][x - i] : false;
			occ += seats[y - i][x - i] === '#' ? 1 : 0;
		}
		if (!found.ur && y - i >= 0 && x + i < cols) {
			found.ur =
				seats[y - i][x + i] !== '.' ? seats[y - i][x + i] : false;
			occ += seats[y - i][x + i] === '#' ? 1 : 0;
		}
		if (!found.dl && y + i < rows && x - i >= 0) {
			found.dl =
				seats[y + i][x - i] !== '.' ? seats[y + i][x - i] : false;
			occ += seats[y + i][x - i] === '#' ? 1 : 0;
		}
		if (!found.dr && y + i < rows && x + i < cols) {
			found.dr =
				seats[y + i][x + i] !== '.' ? seats[y + i][x + i] : false;
			occ += seats[y + i][x + i] === '#' ? 1 : 0;
		}
...
function round() {
	...
	for (let i = 0; i < rows; i++) {
		...
		for (let j = 0; j < cols; j++) {
			...
			newLayout[i][j] = newSeat;
		}
	}
	seats = newLayout;
	// console.log(newLayout, occs);
	if (changes > 0) {
		round();
	} else {
		console.log(occs);
