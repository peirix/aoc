const input = `8 + (9 + 7 * 4 * 9) + 4 * 8 * (6 * 6 * 9 * 8 * 3 * 7) + 2
...
lines.forEach((line) => {
	...
	while (matches = line.match(regex)) {
		matches.forEach((m) => {
			line = line.replace(m, calcB(m));
		});
	}
	if (line.indexOf(' ') > -1) {
		line = parseInt(calcB(line));
	}
	nums.push(line);
	console.log(line);
});
...
function calcB(str) {
	...
	str = str.split(' ');
	let nextAdd;
	while ((nextAdd = str.find((x) => x === '+'))) {
		const ind = str.indexOf('+');
		const num = parseInt(str[ind - 1]) + parseInt(str[ind + 1]);
		str.splice(ind - 1, 3, num.toString());
	}
	while ((nextAdd = str.find((x) => x === '*'))) {
		const ind = str.indexOf('*');
		const num = str[ind - 1] * str[ind + 1];
		str.splice(ind - 1, 3, num.toString());
	}
	return str.join('');
}
